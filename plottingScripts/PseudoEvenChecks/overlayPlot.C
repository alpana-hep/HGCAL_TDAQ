...
void overlayPlot(string root_file_name,string root_file_name1,string map_file, string pathname, string tagname, string tagname1, string tagname2, string outfile)
{
  ...
  if(!mapping_file.is_open())
      ...
      for(int iModule = 0; iModule<length;iModule++)
	{
	  ...
	  resp->SetLineColorAlpha(kRed,1);
	  resp->SetMarkerColor(kRed);
	  resp->SetLineWidth(2);
	  resp1->SetLineColorAlpha(kBlack,1);
	  resp1->SetMarkerColor(kBlack);
