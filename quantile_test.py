...
fout1 = ROOT.TFile("test1.root", 'RECREATE')
...
orig_modules_arrays=np.empty((5,100000),dtype='int32')
Fit_modules_arrays=np.empty((6,100000),dtype='int32')
corr_modules_arrays=np.empty((6,100000),dtype='int32')
mu =[48.1594,117.193,43.0986,10.6816,23.182]
sigma =[7.33409,15.0262,11.0286,3.83365,6.10949]
for im in range(5):    
    s = np.random.normal(mu[im], sigma[im], 100000)
    orig_modules_arrays[im]=s

print(np.cov(orig_modules_arrays))

print(np.corrcoef(orig_modules_arrays))
